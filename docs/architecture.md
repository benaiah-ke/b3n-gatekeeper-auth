...
GateKeeper is an API-first authorization server, identity store, session/device
manager, API-key manager, token issuer, and verification toolkit.

## Runtime Components

- `gatekeeper-api`: FastAPI service exposing direct auth APIs, OAuth/OIDC,
  token verification, admin APIs, API-key/token APIs, MCP metadata, health, and
  version endpoints.
...
- Users own identities, credentials, sessions, devices, refresh-token families,
  and personal tokens.
